...
import { IApiResponse, IProduct, IProductFilter, ICart, IOrder, ICheckoutForm, ICategory, IReview } from '../types';
...
export const productService = {
  getAll: async (filters: IProductFilter = {}) => {
    ...
    const response = await api.get<IApiResponse<IProduct[]>>(`/products?${params}`);
  ...
  getFeatured: async () => {
    const response = await api.get<IApiResponse<IProduct[]>>('/products/featured');
    return response.data.data || [];
  },
  ...
  update: async (id: string, data: Partial<IProduct>) => {
    const response = await api.put<IApiResponse<IProduct>>(`/products/${id}`, data);
    return response.data.data!;
  },
...
export const categoryService = {
  ...
  create: async (data: Partial<ICategory>) => {
    const response = await api.post<IApiResponse<ICategory>>('/categories', data);
    return response.data.data!;
  },

  update: async (id: string, data: Partial<ICategory>) => {
    const response = await api.put<IApiResponse<ICategory>>(`/categories/${id}`, data);
    return response.data.data!;
  },
...
export const cartService = {
  ...
  add: async (productId: string, quantity: number, variant?: string) => {
    const response = await api.post<IApiResponse<ICart>>('/cart/add', { productId, quantity, variant });
    return response.data.data!;
  },

  update: async (productId: string, quantity: number) => {
    const response = await api.put<IApiResponse<ICart>>(`/cart/item/${productId}`, { quantity });
    return response.data.data!;
  },

  remove: async (productId: string) => {
    const response = await api.delete<IApiResponse<ICart>>(`/cart/item/${productId}`);
    return response.data.data!;
  },
...
export const orderService = {
  create: async (data: ICheckoutForm & { items: { productId: string; quantity: number; variant?: string }[] }) => {
  ...
  getMyOrders: async (params: { page?: number; limit?: number; status?: string } = {}) => {
    ...
    const response = await api.get<IApiResponse<IOrder[]>>(`/orders/my-orders?${query}`);
    return response.data;
  },
  ...
  uploadPaymentProof: async (id: string, paymentProof: string) => {
    const response = await api.patch<IApiResponse<IOrder>>(`/orders/${id}/payment-proof`, { paymentProof });
    return response.data.data!;
  },
  ...
  updateStatus: async (id: string, data: Record<string, unknown>) => {
    const response = await api.patch<IApiResponse<IOrder>>(`/orders/${id}/status`, data);
    return response.data.data!;
  },
};
...
export const reviewService = {
  getByProduct: async (productId: string) => {
    const response = await api.get<IApiResponse<IReview[]>>(`/reviews/product/${productId}`);
    return response.data.data || [];
  },

  create: async (data: { productId: string; rating: number; title: string; comment: string; images?: string[] }) => {
...
export const dashboardService = {
  ...
  getMonthlyRevenue: async (year?: number) => {
    const response = await api.get(`/dashboard/revenue/monthly${year ? `?year=${year}` : ''}`);
    return response.data.data;
  },
